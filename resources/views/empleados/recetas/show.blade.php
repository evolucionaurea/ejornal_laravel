...
@section('title', 'Receta #' . $receta->id)
            ...
            <div class="container-fluid py-3">
                ...
                <div class="tarjeta mb-3">
                    <div class="card-body d-flex flex-wrap justify-content-between align-items-start">
                        ...
                        {{-- ACCIONES: se mantienen tal cual para JS y estilos --}}
                        <div id="receta-actions" data-id="{{ $receta->id }}"
                            data-url-anular="{{ route('empleados.recetas.anular', $receta->id) }}"
                            data-csrf="{{ csrf_token() }}">

                            <a href="{{ route('empleados.recetas') }}" class="btn-ejornal btn-ejornal-gris-claro mr-2">
                                ← Volver
                            </a>
                            ...
                            @if (strtolower($receta->estado) !== 'anulada')
                                @php
                                    ...
                                    $permiteAnulacion =
                                        !empty($idFinanciadorReceta) &&
                                        in_array($idFinanciadorReceta, $financiadoresAnulacionIds ?? [], true);
                                @endphp
                                @if ($permiteAnulacion)
                                    <button type="button" class="btn btn-danger text-white btn-sm js-anular">
                                        Anular
                                    </button>
                                @else
                                    <span class="badge badge-secondary p-2"
                                        title="El financiador de esta receta no permite anulación vía API">
                                        No anulable
                                    </span>
                                @endif
                            @endif
                ...
                <div class="tarjeta">
                    <div class="card-body">
                        ...
                        {{-- 1) Fila de resumen: Paciente / Médico / Datos receta --}}
                        <div class="row">
                            {{-- Paciente --}}
