    ...
    public function index()
    {
        ...
        foreach (File::files($logDir) as $file) {
            if (preg_match('/laravel-(\d{4}-\d{2}-\d{2})\.log$/', $file->getFilename(), $m)) {
                ...
                if ($date->greaterThanOrEqualTo($cutoff)) {
        ...
        $cards = [
            ...
            ['label'=>'Memoria total',  'value'=>$memTotalMB,    'icon'=>'hdd','unit'=>' MB'],
            ['label'=>'Memoria usada',  'value'=>$memUsedMB,     'icon'=>'hdd','unit'=>' MB'],
            ['label'=>'Disco total','value'=>$diskTotalMB,'icon'=>'database','unit'=>' MB'],
        ['label'=>'Disco usado','value'=>$diskUsedMB,'icon'=>'chart-pie','unit'=>' MB'],
        ['label'=>'Disco libre','value'=>$diskFreeMB,'icon'=>'chart-bar','unit'=>' MB'],
